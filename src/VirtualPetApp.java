...
//Game loop. User can feed, give water to, or play with Rupert. 
		do {
			rhino.tick(); //Time passes each time this loop is run, increasing hunger, thirst, and boredom.
			...
			if (rhino.hunger >= 80) {
				writeLine("Your rhino is getting hungry..."); //Warning that Rupert is growing hungry
			if (rhino.thirst >=80) {
				writeLine("Your rhino is dangerously parched.."); //Warning that Rupert is growing thirsty
			}
			if (rhino.boredom >= 80) {
				writeLine("Rhinos can die of boredom... "); //Warning that Rupert is getting bored
			}
			if (rhino.hunger<=-20){
			writeLine("You may be overfeeding your rhino..."); //Warning that user is overfeeding the rhino
			}
			}
		} while (rhino.hunger <= 100 || rhino.thirst <= 100 || rhino.boredom <= 100); //Reaching these levels causes death and ends the game.
